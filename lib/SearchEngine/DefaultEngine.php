...
class DefaultEngine extends Contract
{
    ...
    function buildSQL(): array
    {
        ...
        $sql_group = '';
        if (
            !is_null($this->criteria->location) || !is_null($this->criteria->colltype) || !is_null($this->criteria->side_author)
            || !is_null($this->filter->location) || !is_null($this->filter->colltype) || !is_null($this->filter->side_author)
    ...
    function buildCriteria($criteria): string
    {
        ...
        foreach ($criteria->toCQLToken($this->stop_words) as $token) {
            ...
            // search value
            $query = $token['q'] ?? null;
            $query = !in_array($field, ['location', 'colltype', 'gmd', 'attachment', 'lang', 'side_author']) ? str_replace(['\\', '"', '\''], '', $query) : $query;
            switch ($field) {
                case 'title':
                    $this->execute[] = "%" . $query . "%";
                ...
                case '_side_author':
                    ...
                    if (is_null($idx)) {
                        $this->execute[] =  $query;
                        $sub_query = "select ba.biblio_id from biblio_author as ba left join mst_author as ma on ba.author_id=ma.author_id where ma.author_name like ?";
                        // $sub_query = "select author_id from mst_author where author_name = ?";
                    } else {
                        $this->execute = array_merge($this->execute, $idx);
                        $sub_query = trim(str_repeat('?,', count($idx ?? 1)), ',');
                    }
                ...
                case 'side_author':
                    ...
                    // If $idx is not an array, we treat it as a single value.
                    if (is_null($idx)) {
                        $this->execute[] = "%" . $query . "%";
                        $sub_query = "select ba.biblio_id from biblio_author as ba left join mst_author as ma on ba.author_id=ma.author_id where ma.author_name like ?";
                    } else {
                        $this->execute = array_merge($this->execute, array_map(fn($q) => "%" . $q . "%", $idx));
                        $sub_query = "select ba.biblio_id from biblio_author as ba left join mst_author as ma on ba.author_id=ma.author_id where "
                            . implode(' OR ', array_fill(0, count($idx), 'ma.author_name like ?'));
                    }
                ...
                case 'publishyear':
                    ...
                    case 'series_title':
                        $this->execute[] = '%' . $query . '%';
                        if ($bool === '-') {
                            $sql_criteria .= " b.series_title NOT LIKE ?";
                        } else {
                            $sql_criteria .= " b.series_title LIKE ?";
                        }
                        break;
