...
HTML;
    ...
    public function getItemCopy() {
      ...
      $_output = '<div class="flex flex-col"> <table  class="table table-bordered col-md-12">
                      <thead>
                          <tr>
                            <th>လက်ရှိနေရာ</th>
                            <th>စာအုပ်စင်နေရာ</th>
      ...
      foreach ($copies as $copy_d) {
        ...
        $coll_type_name = empty($coll_type_name) ? __('Collection Type is not set') : $coll_type_name;

        // if (trim($copy_d['site']) != "") {
          // $location_name .= ' ('.$copy_d['site'].')';
          $shelf_location = trim($copy_d['site']);
          $shelf_location_name = empty($shelf_location) ? __('-') : $shelf_location;
        // }

        $call_number = empty($call_number) ? __('-') : $call_number;
        $_output .= <<<HTML
            ...
            <tbody>
                    <tr>
                      <td>{$location_name}</td>
                      <td>{$shelf_location_name}</td>
                      <td>{$coll_type_name}</td>
                      <td>{$item_code}</td>
                      <td>{$call_number}</td>
                      <td > <b class="text-sm availability-item {$avail_class}">{$avail_status}</b></td>
                    </tr>
                </tbody>
    ...
    protected function htmlOutput()
    {
        ...
        foreach ($this->record_detail['authors'] as $data) {
          // Edit By Ye Htun
          if($data['authority_type'] == 'Organizational Body'){
             $organization_name[] = $data['author_name'];
          }
          elseif($data['authority_type'] == 'Personal Name')
          {
          $author_prefix[] = $data['author_prefix'];
            $personal_name[] = $data['author_name'];
          }
          elseif($data['authority_type'] == 'Conference')
          {
            $conference[] = $data['author_name'];
          }

          $authors .= '<a href="?author='.urlencode('"'.$data['author_name'].'"').'&search=Search" title="'.__('Click to view others documents with this author').'">'.$data['author_prefix'].'၊  '.$data['author_name']."</a> ။ ";
        ...
        $this->record_detail['authors'] = $authors;
          ...
          $this->record_detail['organization_name'] = empty($organization_name) ? __('Orgination name is not set') : $organization_name;
          $this->record_detail['personal_name'] = empty($personal_name) ? __('-') : $personal_name;
          $this->record_detail['conference'] = empty($conference) ? __('-') : $conference;
          $this->record_detail['author_prefix'] = empty($author_prefix) ? __('-') : $author_prefix;
