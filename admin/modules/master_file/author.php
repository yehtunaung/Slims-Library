...
if (isset($_POST['saveData']) AND $can_read AND $can_write) {
    ...
    $authorPrefix = trim(strip_tags($_POST['authorPrefix']));
    // check form validity
    if (empty($authorName)) {
        ...
        $data['author_prefix'] = $dbs->escape_string($authorPrefix);
...
if (isset($_POST['detail']) OR (isset($_GET['action']) AND $_GET['action'] == 'detail')) {
    ...
    /* Form Element(s) */
    // author name
    $form->addTextField('text', 'authorPrefix', __('Author Prefix').'*', $rec_d['author_prefix']??'', ' class="form-control col-6"');
    ...
    if ($can_read AND $can_write) {
        $auth_type_fld = 3;
        $datagrid->setSQLColumn('a.author_id', 'a.author_prefix AS \''.__('Author Prefix').'\'','a.author_name AS \''.__('Author Name').'\'',
        ...
        $datagrid->setSQLColumn('a.author_prefix AS \''.__('Author Prefix').'\'','a.author_name AS \''.__('Author Name').'\'',
            'a.author_year AS \''.__('Author Year').'\'',
            'a.authority_type AS \''.__('Authority Type').'\'',
            'a.auth_list AS \''.__('Authority Files').'\'',
            'a.last_update AS \''.__('Last Update').'\'');
