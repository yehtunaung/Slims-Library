...
class Biblio
{
  ...
  public function getItemCopy()
  {
    $items = array();
    $_output = '';
    $copy_q = $this->db->query(sprintf('SELECT i.item_code, colltype.coll_type_name,i.call_number, loc.location_name, stat.*, i.site FROM item AS i
        LEFT JOIN mst_item_status AS stat ON i.item_status_id=stat.item_status_id
        LEFT JOIN mst_coll_type AS colltype ON i.coll_type_id=colltype.coll_type_id
  ...
  public function getAuthors()
  {
    ...
    $_biblio_authors_q = $this->db->query(sprintf('SELECT author_prefix,author_name, authority_type, auth_list, level FROM mst_author AS a'
