...
if (isset($_POST['save']) AND (isset($_POST['authorID']) OR trim($_POST['search_str']))) {
  ...
  $author_prefix = trim($dbs->escape_string(strip_tags($_POST['author_prefix'])));
  ...
  if (isset($_POST['biblioID']) AND !empty($_POST['biblioID'])) {
      ...
      if (isset($_POST['authorID']) AND !empty($_POST['authorID'])) {
          ...
          if ($author_id !== false) {
              ...
              $author_data['author_prefix'] = $author_prefix;
      ...
      if (isset($_POST['authorID']) AND !empty($_POST['authorID'])) {
          ...
          if ($author_id !== false) {
              ...
              // adding new author
              $data['author_prefix'] = $author_prefix;
...
<div class="popUpForm">
    ...
    <form name="mainForm" action="pop_author.php?biblioID=<?php echo $biblioID; ?>" method="post">
        <div class="s-margin__bottom-1 form-inline">
            <?php
            $ajax_exp = "ajaxFillSelect('../../AJAX_lookup_handler.php', 'mst_author', 'author_id:author_prefix:author_name:author_year:authority_type', 'authorID', $('#search_str').val())";?>
            <input type="text" name="author_prefix" placeholder="<?php echo __('Author Prefix') ?>" class="form-control col-md-6" id="search_str"  oninput="<?php echo $ajax_exp; ?>" />
            <input type="text" name="search_str" placeholder="<?php echo __('Author Name') ?>" class="form-control  col-md-6" id="search_str" oninput="<?php echo $ajax_exp; ?>" />
            <select name="type" class="col-md-8 form-control"><?php
            foreach ($sysconf['authority_type'] as $type_id => $type) {
                echo '<option value="'.$type_id.'">'.$type.'</option>';
            }
            ?></select>
            <select name="level" class="col-md-6 form-control mt-3" ><?php
