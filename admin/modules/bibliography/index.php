...
$rda_cmc = array('content' => 'Content Type', 'media' => '##942 <span class="mc_pre"> </span>Media Type', 'carrier' => 'Carrier Type');
...
if (isset($_POST['saveData']) AND $can_read AND $can_write) {
    if (!simbio_form_maker::isTokenValid()) {
        ...
        utility::writeLogs($dbs, 'staff', $_SESSION['uid'], 'system', 'Invalid form submission token, might be a CSRF attack from ' . $_SERVER['REMOTE_
        R']);
        exit();
    }
    ...
    if (empty($title)) {
        ...
        // if(trim($dbs->escape_string(strip_tags($_POST['callNumber']))) == "သတင်းစာများ")
        // {
...
if (isset($_GET['action']) && $_GET['action'] == 'history') {
    ...
    /* Form Element(s) */
    // biblio title
    $form->addTextField('textarea', 'title', __('#245 (a) <span class="mc_pre"></span>  Title') . '*', $rec_d['title'] ?? '', 'rows="1" class="form-control"',
    ...
    $form->addAnything(__('##100 <span class="mc_pre"></span> Author(s)'), $str_input);
    ...
    // biblio edition
    $form->addTextField('text', 'edition', __('##250 <span class="mc_pre"> Edition'), $rec_d['edition'] ?? '', 'class="form-control" style="width: 50%;"', __('A version of publication having substantial changes or additions.'));
    // biblio specific detail info/area
    $form->addTextField('textarea', 'specDetailInfo', __('##910 <span class="mc_pre"> Specific Detail Info'), $rec_d['spec_detail_info'] ?? '', 'rows="2" class="form-control', __('explain more details about an item e.g. scale within a map, running time in a movie dvd.'));
  ...
  <div class="bnt btn-group"><div class="btn btn-info" data-toggle="collapse" data-target="#batchItemDetail" aria-expanded="false" aria-controls="batchItemDetail">' . __('Options') . '</div>';
    ...
    // biblio ISBN/ISSN
    $form->addTextField('text', 'isbn_issn', __('##020 <span class="mc_pre"></span> ISBN/ISSN'), $rec_d['isbn_issn'] ?? '', 'class="form-control" style="width: 40%;"', __('Unique publishing number for each title of publication.'));
    // biblio publisher
    ...
    $form->addSelectList(
        'callNumber', // Name of the select list
        __('Resource Collection'), // Label for the select list
        $callNumberSelect, // The options for the select list
        $rec_d['call_number'] ?? '', // No pre-selected value
        'class="select2"', // Additional attributes
        __('RDA designation.') // Help text
    );
