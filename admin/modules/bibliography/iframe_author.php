...
if ($biblioID) {
  ...
  // database list
  $biblio_author_q = $dbs->query("SELECT ba.*, a.author_prefix,a.author_name, a.author_year, a.authority_type FROM biblio_author AS ba
  ...
  while ($biblio_author_d = $biblio_author_q->fetch_assoc()) {
    ...
    $author_prefix = $biblio_author_d['author_prefix']; //Edit By Ye Htun
    ...
    $table->appendTableRow(array($edit_link.$remove_link,$author_year,$author_prefix, $author, $authority_type,$authority_level_link));
    $table->setCellAttr($row, 0, 'class="'.$row_class.'" style="font-weight: bold; width: 10%;"');
    $table->setCellAttr($row, 1, 'class="'.$row_class.'" style="width: 20%;"');
    ...
    $table->setCellAttr($row, 5, 'class="'.$row_class.'" style="width: 20%;"');
    $row++;
  }
  }
  ...
  if ($_SESSION['biblioAuthor']) {
    ...
    foreach ($_SESSION['biblioAuthor'] as $biblio_session) {
      ...
      if ($biblio_session) {
          $author_q = $dbs->query("SELECT author_prefix,author_name, author_year, authority_type FROM mst_author
      ...
      $table->appendTableRow(array($remove_link,$author_prefix, $author, $authority_type, $sysconf['authority_level'][$biblio_session[1]]));
